...
fn parse_contents(contents: String) -> Vec<(char, i32)> {
    contents.split(", ").map(|x| {
        let mut c = x.chars();
        (c.next().unwrap(), dbg!(c.collect::<String>()).parse::<i32>().unwrap())
    }).collect()
}
...
fn take_step(num_steps: i32, position: &mut (i32, i32), seen_positions: &mut HashSet<(i32, i32)>, direction: char) -> Option<(i32, i32)> {
